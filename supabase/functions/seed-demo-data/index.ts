import { bearerUser, options, originAllowed, reply, serviceAuth, sql } from '../_shared/security.ts'
...
type SeedBloodRequest = { patient_name: string; blood_group: string; hospital: string; mobile: string; address: string }

const randomPassword = () => {
  const bytes = crypto.getRandomValues(new Uint8Array(24))
  return `${[...bytes].map(value => value.toString(16).padStart(2, '0')).join('')}aA1!`
}

Deno.serve(async request => {
  if (request.method === 'OPTIONS') return originAllowed(request) ? options() : reply(generic, 403)
  if (request.method !== 'POST' || !originAllowed(request)) return reply(generic, 403)
  ...
  try {
    ...
    const bloodRequests: SeedBloodRequest[] = Array.isArray(body.blood_requests) ? body.blood_requests : []
    if (!requestId || !uuid.test(requestId) || label.length < 1 || label.length > 160 || reason.length < 3 || reason.length > 500 || users.length > MAX_USERS || bloodRequests.length > MAX_BLOOD_REQUESTS || (users.length === 0 && bloodRequests.length === 0)) return reply(generic, 400)

    const normalizedUsers = users.map(user => ({
      email: typeof user?.email === 'string' ? user.email.trim().toLowerCase() : '',
      role: typeof user?.role === 'string' ? allowedRoles.get(user.role.trim().toLowerCase()) : undefined,
    }))
    if (normalizedUsers.some(user => !emailPattern.test(user.email) || user.email.length > 320 || !user.role) || new Set(normalizedUsers.map(user => user.email)).size !== normalizedUsers.length) return reply(generic, 400)
    ...
    const normalizedRequests = bloodRequests.map(item => {
      ...
      return {
        ...
        mobile: digits.length === 12 && digits.startsWith('91') ? digits.slice(2) : digits,
        address: String(item?.address ?? '').trim(),
      }
    })
    if (normalizedRequests.some(item => item.patient.length < 2 || item.patient.length > 100 || item.hospital.length < 2 || item.hospital.length > 160 || item.address.length < 5 || item.address.length > 500 || !/^[6-9]\d{9}$/.test(item.mobile) || !bloodGroups.has(item.blood))) return reply(generic, 400)

    batchId = crypto.randomUUID()
    return await sql.begin(async transaction => {
      await transaction`select pg_advisory_xact_lock(hashtextextended(${requestId}, 0))`

      const actorRows = await transaction`select role,status from public.users where user_id=${actor.id}`
      if (actorRows.length !== 1 || String(actorRows[0].status).toLowerCase() !== 'active') return reply(generic, 403)
      ...
      for (const seedUser of normalizedUsers) {
        ...
        const { data, error } = await serviceAuth.auth.admin.createUser({
          ...
          email_confirm: true,
          app_metadata: { is_demo: true, demo_batch_id: batchId, demo_membership_id: membershipId },
        })
        if (error || !data.user) throw new Error('Demo Auth identity creation failed')
        ...
        const mapped = await transaction`
          ...
          returning user_id`
        if (mapped.length !== 1) throw new Error('Demo public.users mapping was not unique')
        await transaction`
      ...
      await transaction`
        ...
        values ('demo_seed_completed',${actor.id},'demo_batch',${batchId},${reason},${JSON.stringify({ request_id: requestId, users: normalizedUsers.length, blood_requests: normalizedRequests.length })}::jsonb)`
      return reply({ completed: true, batch_id: batchId })
    })
  } catch {
    let cleanupFailed = false
    for (const authUserId of createdAuthUserIds.reverse()) {
      const { error } = await serviceAuth.auth.admin.deleteUser(authUserId, false)
    ...
    if (actorUserId) {
      try {
        await sql`
          insert into security.authorization_audit_log (event_type,actor_user_id,target_type,target_id,reason,metadata)
          values ('demo_seed_failed',${actorUserId},'demo_batch',${batchId},${cleanupFailed ? 'transaction failed; Auth compensation incomplete' : 'transaction failed; created Auth identities compensated'},${JSON.stringify({ request_id: requestId, created_auth_users: createdAuthUserIds.length, cleanup_failed: cleanupFailed })}::jsonb)`
      } catch { /* Keep the response generic when failure auditing is unavailable. */ }
    }
