...
for case_file in negative-duplicate.sql negative-unmatched.sql negative-privileged-null.sql; do
  if "$psql_bin" "$BCI_DATABASE_URL" --no-psqlrc --set=ON_ERROR_STOP=1 --file "supabase/tests/integration/ci/$case_file" >/dev/null 2>&1; then
    echo '{"phase":"identity-negative","status":"failed","reason":"expected-failure-did-not-occur"}' >&2
    exit 1
  fi
  "$psql_bin" "$BCI_DATABASE_URL" --no-psqlrc --set=ON_ERROR_STOP=1 --file supabase/tests/integration/ci/verify-negative-cleanup.sql >/dev/null
done
