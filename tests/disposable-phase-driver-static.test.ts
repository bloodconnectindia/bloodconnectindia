...
const [baseline, adapter, driver, cleanup, workflow, config] = await Promise.all([
...
Deno.test("baseline is minimal, guarded, and live-aligned", () => {
  for (const relation of ["public.users", "public.donors", "public.blood_requests", "public.blood_stock", "public.blood_banks", "public.hospitals"]) {
    if (!baseline.includes(relation)) throw new Error(`Missing baseline relation: ${relation}`);
  }
  for (const column of ["user_id text", "role text", "status text", "patient_name text", "blood_group text", "hospital text", "mobile text", "address text"]) {
    if (!baseline.includes(column)) throw new Error(`Missing verified baseline column: ${column}`);
  }
  for (const forbidden of ["public.profiles", "public.user_roles", "create table auth.users"]) {
    if (baseline.toLowerCase().includes(forbidden)) throw new Error(`Forbidden baseline assumption: ${forbidden}`);
  }
  if (/^\s*demo_batch_id\s+/mi.test(baseline)) throw new Error("Baseline prematurely defines demo_batch_id");
});

Deno.test("adapter uses native Auth and reserved fake identities only", () => {
  if (!adapter.includes("insert into auth.users") || !adapter.includes("@bci.invalid")) throw new Error("Native fake Auth adapter missing");
  for (const forbidden of ["password", "service_role", "supabase.co"]) if (adapter.toLowerCase().includes(forbidden)) throw new Error(`Unsafe adapter material: ${forbidden}`);
});

Deno.test("phase allowlist and exact controlled order are present", () => {
  const phases = ["start-local-stack", "baseline", "schema-preflight", "identity-negative", "identity-clean", "identity-foundation", "identity-evidence", "identity-index", "authorization-migration", "authorization-verification", "demo-migration", "demo-verification", "auth-fixtures", "edge-functions", "concurrency-replay", "password-recovery"];
  const dispatcher = driver.slice(driver.lastIndexOf('case "$phase" in'));
  let position = -1;
  for (const phase of phases) {
    const next = dispatcher.indexOf(`${phase})`);
    if (next <= position) throw new Error(`Phase missing or out of dispatcher order: ${phase}`);
    position = next;
  }
  if (!driver.includes("unknown-phase") || !driver.includes("exit 64")) throw new Error("Unknown phase does not fail closed");
});

Deno.test("driver is approval-gated and loopback-only", () => {
  for (const required of ["PHASE_DRIVER_APPROVED", "approval-marker-missing", "BCI_DISPOSABLE_APPROVAL", "127.0.0.1", "54322", "/postgres", "remote-link-state"]) {
    if (!driver.includes(required)) throw new Error(`Missing driver guard: ${required}`);
  }
  if (!config.includes('project_id = "bloodconnectindia-disposable-ci"')) throw new Error("Disposable project ID missing");
});
...
Deno.test("no remote-management command, remote URL, or unsanitized output primitive exists", () => {
  ...
  for (const forbidden of ["supabase " + "login", "supabase " + "link", "supabase db " + "push", "supabase functions " + "deploy", "supabase.co", "printenv", "env |", "set -x"] ) {
    if (combined.includes(forbidden)) throw new Error(`Forbidden executable content: ${forbidden}`);
  }
  if (!driver.includes("result()") || !driver.includes('"status"')) throw new Error("Structured phase result missing");
});
